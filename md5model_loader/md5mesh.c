		...
		else if (strncmp (buff, "mesh {", 6) == 0)
		{
			...
			while ((buff[0] != '}') && !feof (fp))
			{
				...
				if (strstr (buff, "shader "))
				{
					...
					mesh->texid = SOIL_load_OGL_texture(mesh->shader, SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS);
					printf("texture: %s\n", mesh->shader);
					assert(mesh->texid);
					printf("\nTexid %d\n", mesh->texid);
				}
				else if (sscanf (buff, " numverts %d", &mesh->num_verts) == 1)
				{
					if (mesh->num_verts > 0)
					{
						...
						mesh->vertexArray = (struct vec3_t *)malloc(sizeof(vec3_t) * mesh->num_verts);
						mesh->tex_coords = (vec2_t *)malloc(sizeof(vec2_t) * mesh->num_verts);
					}
				...
				else if (sscanf (buff, " numtris %d", &mesh->num_tris) == 1)
				{
					if (mesh->num_tris > 0)
					{
						...
						mesh->vertexIndices = (GLuint *)malloc(sizeof(GLuint) * 3 * mesh->num_tris);
					}
			...
			for (int j = 0; j < mesh->num_verts; j++) {
				mesh->tex_coords[j][0] = mesh->vertices[j].st[0];
				mesh->tex_coords[j][1] = mesh->vertices[j].st[1];
			}

			for (int j = 0; j < mesh->num_verts; j++) {
				printf("u: %f  v: %f\n", mesh->tex_coords[j][0], mesh->tex_coords[j][1]);
			}
			curr_mesh++;
		}
	}
...
void PrepareMesh (const struct md5_mesh_t *mesh, const struct md5_joint_t *skeleton)
{
	...
	for (k = 0, i = 0; i < mesh->num_tris; ++i) {
		for (j = 0; j < 3; ++j, ++k)
			mesh->vertexIndices[k] = mesh->triangles[i].index[j];
	}
	...
	for (i = 0; i < mesh->num_verts; ++i) {
		...
		mesh->vertexArray[i][0] = finalVertex[0];
		mesh->vertexArray[i][1] = finalVertex[1];
		mesh->vertexArray[i][2] = finalVertex[2];
    }
}
...
void display ()
{
	...
	angle += 0.1 * (curent_time - last_time);
	...
	//DrawSkeleton (skeleton, md5file.num_joints); //Draw skeleton
	...
	for (i = 0; i < md5file.num_meshes; ++i) {
		...
		glBindTexture(GL_TEXTURE_2D, md5file.meshes[i].texid);
		glVertexPointer (3, GL_FLOAT, 0, md5file.meshes[i].vertexArray);
		//glNormalPointer(GL_FLOAT, 0, &_normalArray.front());
		glTexCoordPointer(2, GL_FLOAT, 0, md5file.meshes[i].tex_coords);
		glDrawElements (GL_TRIANGLES, md5file.meshes[i].num_tris * 3, GL_UNSIGNED_INT, md5file.meshes[i].vertexIndices);
...
int main (int argc, char *argv[])
{
	...
	/* Load MD5 model file */
	if (!ReadMD5Model("untitled.md5mesh", &md5file))
		exit(EXIT_FAILURE);
	...
	animated = 0;
	//if (!ReadMD5Anim("boblampclean.md5anim", &md5anim))
	//{
	//	FreeAnim(&md5anim);
	//}
	//else
	...
	//	skeleton = (struct md5_joint_t *)malloc(sizeof(struct md5_joint_t) * md5anim.num_joints);
	//	animated = 1;
	//}
	///////////////////////////////////////////////////////////////////
