...
#define CHUNK_WIDTH 16
	chunk.Init(vec3int(0, 0, 0), CHUNK_WIDTH, CHUNK_WIDTH, VOXEL_FLAG_SOLID);
	chunk.DebugDraw_ChunkBounds(true);
	printf("Chunk info:  Width: %d   Height: %d  vmin(%d %d %d) vmax(%d %d %d)\n",
		chunk.m_nWidth, chunk.m_nHeight,
		chunk.m_ChunkPos.x, chunk.m_ChunkPos.y, chunk.m_ChunkPos.z,
		chunk.m_vecMax.x, chunk.m_vecMax.y, chunk.m_vecMax.z);

	for (int y = CHUNK_WIDTH / 2; y < CHUNK_WIDTH; y++)
		for (int x = CHUNK_WIDTH / 2; x < CHUNK_WIDTH; x++)
			for (int z = CHUNK_WIDTH / 2; z < CHUNK_WIDTH; z++)
				chunk.VoxelAt(x, y, z)->SetFlag(VOXEL_FLAG_AIR);
...
void fn_mouseclick(HWND hWnd, int x, int y, int button, int state)
{
	if (state == KEY_DOWN) {
		if (button == LBUTTON) {
			if (chunk.DestroyVoxel(ray, 10.f, VOXEL_FLAG_AIR)) {
