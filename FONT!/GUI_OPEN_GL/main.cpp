...
{
	fontmanager.Init();
	p_font = fontmanager.LoadFont("ARIALNBI.TTF", FONTTYPE_TTF, 18);
	if (!p_font) {
		char error[128];
		sprintf_s(error, sizeof(error), "Failed to load font!\nError: %d", fontmanager.LastError());
		MessageBoxA(hWnd, error, "Error", 0);
	}

	//LoadTexture("notex.bmp", &g_texture);
	ppbs = new CProgressBar(100, 50, 120, 8, 66, { 255,255,255 }, { 246,145,205 });
	ppb = new CButton(100, 100, 120, 45, { 60, 60, 60 }, NULL, NULL);
